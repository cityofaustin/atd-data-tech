---
name: "[Tech Services] New Hire Onboarding Checklist - Tech Services Support"
about: This template is to help the Tech Services team with onboarding new TPW employees
title: "[Tech Services] New Hire Onboarding Checklist - [START DATE: FIRST NAME, LAST
  INITIAL, LOCATION] "
labels: 'Team: Tech Services, Type: Operations, [Workgroup] :rotating_light: MISSING'
assignees: ''

---

## Onboarding Request
- Effective Start Date: 
- Employee Type: 
- Title: 
- New Employee Name: First Name, Last Initial <!-- LASTNAME -->
- Whose account to mirror: First Name, Last Initial <!-- LASTNAME -->
- Supervisor: First Name, Last Initial <!-- LASTNAME -->
- Division: 
- Work Location: 

Reference - [New Hire Checklist document](https://cityofaustin.sharepoint.com/:w:/r/sites/tpwDTS/_layouts/15/Doc.aspx?sourcedoc=%7B81512810-B700-448C-8200-38B343CB257D%7D&file=New%20Hire.docx&action=default&mobileredirect=true)
- [ ] Check if NERF was submitted?
- [ ] Set up password/Reset password (Ask Scott & Matt, if available, else ask CTM help desk (512-974-HELP)

## Hardware Checklist
>Does the user need a GIS Modeling computer?
- [ ]  GIS Laptop w/ docking station + power supply
- [ ]  Laptop + power supply

### Request peripherals (If desk space does not come equipped)
- [ ]  Docking station
- [ ]  Monitors
- [ ]  Keyboard (hardwired)
- [ ]  Mouse (hardwired)
*Headphones
DTS Tech Services does not supply as we do not have them regularly on-hand (Please talk to your supervisor and use the office supply contract)

## Software Checklist
- [ ]  Adobe Pro
- [ ]  ArcGIS 
- [ ]  AutoCAD/AutoCAD 3D
- [ ]  BlueBeam 
- [ ]  SketchUp 
- [ ]  MS Project
- [ ]  MS Visio

### Business Justification
- [copy and paste justification]

### Funding Info
